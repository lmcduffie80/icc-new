...
import { adminDocumentUploadSchema } from '@/lib/validation';

// POST: Generate presigned URL for product document (PDF) upload
export async function POST(request: NextRequest) {
  ...
  if (!hasPermission) {
    securityLogger.logPermissionDenied(
      session.user.id,
      session.user.email,
      '/api/admin/products/upload-document',
  ...
  try {
    ...
    const validationResult = adminDocumentUploadSchema.safeParse({
    ...
    // Generate unique key for product documents
    const extension = sanitizedFileName.split('.').pop() || 'pdf';
    const key = `product-documents/${Date.now()}-${sanitizedFileName.replace(/\.[^/.]+$/, '')}.${extension}`;
    ...
    console.error('Error generating product document upload URL:', error);
    
    securityLogger.logEvent({
      type: 'suspicious_activity',
      userId: session.user.id,
      ip,
      path: '/api/admin/products/upload-document',
      method: 'POST',
