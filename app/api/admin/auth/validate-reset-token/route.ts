...
/**
 * GET /api/admin/auth/validate-reset-token?token={token}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      securityLogger.logRateLimitExceeded(ip, '/api/admin/auth/validate-reset-token', 'GET');
    ...
    const tokenRecord = await queryOne<TokenRecord>(
      `SELECT rt.token, rt.expires_at, rt.used_at, au.email
    ...
    securityLogger.logEvent({
      ...
      ip,
      path: '/api/admin/auth/validate-reset-token',
      method: 'GET',
