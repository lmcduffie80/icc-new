...
import { sendSupplierPasswordResetEmail } from '@/lib/email';

// Generic success message (prevents user enumeration)
const GENERIC_SUCCESS_MESSAGE = 'If a supplier account exists with that email, you will receive a password reset link shortly.';
...
/**
 * POST /api/supplier/auth/request-reset
 * Request password reset for supplier account
 *
 * Security:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      securityLogger.logRateLimitExceeded(ip, '/api/supplier/auth/request-reset', 'POST');
    ...
    if (!result.success) {
      securityLogger.logValidationFailure(
        '/api/supplier/auth/request-reset',
        ip,
        result.error.issues,
        'POST'
      );
    ...
    const emailRateLimitKey = `supplier_reset_email:${email.toLowerCase()}`;
    ...
    if (!emailRateLimit.success) {
      ...
      securityLogger.logEvent({
        ...
        ip,
        path: '/api/supplier/auth/request-reset',
        method: 'POST',
    ...
    // 4. BUSINESS LOGIC
    // Look up supplier user by email
    const supplierUser = await queryOne<SupplierUser>(
      'SELECT id, email, name FROM supplier_users WHERE email = $1',
      [email]
    );
    ...
    await query(
      `INSERT INTO supplier_password_reset_tokens
       (supplier_user_id, token, expires_at, ip_address, user_agent)
       VALUES ($1, $2, $3, $4, $5)`,
      [supplierUser.id, token, expiresAt, ip, userAgent || null]
    );

    // Send password reset email
    const resetUrl = `${process.env.BETTER_AUTH_URL}/supplier/reset-password/${token}`;
    await sendSupplierPasswordResetEmail({
      to: supplierUser.email,
      subject: 'Supplier Portal Password Reset Request',
    ...
    securityLogger.logEvent({
      ...
      username: supplierUser.email,
      ip,
      path: '/api/supplier/auth/request-reset',
      method: 'POST',
      details: { action: 'password_reset_requested', user_type: 'supplier' },
      severity: 'medium',
    });
