...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      securityLogger.logRateLimitExceeded(ip, '/api/supplier/auth/reset-password', 'POST');
    ...
    const tokenRecord = await queryOne<TokenRecord>(
      `SELECT rt.id, rt.supplier_user_id, rt.used_at, rt.expires_at, su.email, su.name
    ...
    if (!tokenRecord) {
      securityLogger.logEvent({
        ...
        ip,
        path: '/api/supplier/auth/reset-password',
        method: 'POST',
    ...
    securityLogger.logEvent({
      type: 'admin_action',
      userId: tokenRecord.supplier_user_id,
      username: tokenRecord.email,
      ip,
      path: '/api/supplier/auth/reset-password',
      method: 'POST',
      details: { action: 'password_reset_completed', user_type: 'supplier' },
      severity: 'high',
    });
