...
import { Save, Loader2, Plus, Trash2, Calendar, MapPin, CheckCircle } from 'lucide-react';
...
interface PurchaseOrder {
  ...
  ship_to_address: AddressFromAPI;
  bill_to_address: AddressFromAPI;
}
...
export function EditPurchaseOrderForm({ purchaseOrder }: EditPurchaseOrderFormProps) {
  ...
  // Format order date for HTML date input (yyyy-MM-dd)
  ...
  const [orderDate, setOrderDate] = useState(formatDateForInput(purchaseOrder.order_date));
  ...
  const [lines, setLines] = useState<LineItem[]>(
    purchaseOrder.lines.map(line => {
      // Format need_by_date properly for date input (yyyy-MM-dd or empty string)
      let formattedDate = '';
      if (line.need_by_date) {
        try {
          // If it's already a string in the correct format, use it
          if (typeof line.need_by_date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(line.need_by_date)) {
            formattedDate = line.need_by_date;
          } else {
            // Otherwise, parse and format it
            const date = new Date(line.need_by_date);
            if (!isNaN(date.getTime())) {
      ...
      return {
        ...
        need_by_date: formattedDate,
        quantity: typeof line.quantity === 'number' ? line.quantity : parseFloat(String(line.quantity)) || 0,
        uom: line.uom,
        unit_price: typeof line.unit_price === 'number' ? line.unit_price : parseFloat(String(line.unit_price)) || 0,
        taxable: line.taxable,
        store_price: line.store_price,
        icc_margin_percent: line.icc_margin_percent,
        icc_margin_amount: line.icc_margin_amount,
        customer_margin_percent: line.customer_margin_percent,
        customer_margin_amount: line.customer_margin_amount,
        supplier_share_amount: line.supplier_share_amount,
      };
    })
  );
  ...
  // Fetch products when vendor/supplier is available
  useEffect(() => {
    const isSupplierPO = !!purchaseOrder.supplier_id;
    const selectedId = isSupplierPO ? purchaseOrder.supplier_id : vendorId;
    ...
    const url = isSupplierPO
      ? `/api/admin/purchase-orders/products?supplier_id=${purchaseOrder.supplier_id}`
  ...
  const validateForm = (): string | null => {
    // Vendor is required only for vendor POs (not supplier POs)
    if (!purchaseOrder.supplier_id && !vendorId) return 'Vendor is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestBody: {
        ...
        tax_rate: typeof taxRate === 'string' ? parseFloat(taxRate) : taxRate,
        status,
        notes: notes.trim() || null,
        lines: lines.map(line => {
          // Format need_by_date: convert to 'yyyy-MM-dd' format or null
          let formattedDate: string | null = null;
          if (line.need_by_date && line.need_by_date.trim()) {
            // If it's already in the correct format, use it
            if (/^\d{4}-\d{2}-\d{2}$/.test(line.need_by_date.trim())) {
              formattedDate = line.need_by_date.trim();
            } else {
          ...
          return {
            line_number: line.line_number,
            item_number: line.item_number.trim(),
            description: line.description.trim(),
            need_by_date: formattedDate,
            quantity: typeof line.quantity === 'string' ? parseFloat(line.quantity) || 0 : line.quantity,
            uom: line.uom.trim(),
            unit_price: typeof line.unit_price === 'string' ? parseFloat(line.unit_price) || 0 : line.unit_price,
            taxable: line.taxable,
          };
        }),
      };

      // Only include vendor_id for vendor POs, supplier_id for supplier POs
      if (purchaseOrder.supplier_id) {
        requestBody.supplier_id = purchaseOrder.supplier_id;
      } else if (vendorId) {
        requestBody.vendor_id = parseInt(vendorId, 10);
      }
  ...
  const subtotal = lines.reduce((sum, line) => {
    if (line.store_price && line.customer_margin_percent !== undefined && line.customer_margin_percent !== null) {
  ...
  // Calculate total supplier share across all lines
  const totalSupplierShare = lines.reduce((sum, line) => {
    if (line.supplier_share_amount !== undefined && line.supplier_share_amount !== null) {
      return sum + (line.quantity * line.supplier_share_amount);
    }
    return sum;
  }, 0);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {success && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-emerald-900">Purchase Order Updated Successfully</p>
            <p className="text-xs text-emerald-700 mt-1">Redirecting to purchase orders list...</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {/* Vendor/Supplier */}
          <div>
            <label htmlFor="vendorSupplier" className="block text-sm font-medium text-slate-700 mb-1">
              {purchaseOrder.supplier_id ? 'Supplier' : 'Vendor'}
            </label>
            {purchaseOrder.supplier_id ? (
              // Show supplier as read-only (suppliers can't be changed after creation)
          ...
          {/* Supplier Address - Display when supplier PO */}
          {purchaseOrder.supplier_id && (() => {
            const hasAddress = purchaseOrder.supplier_address_street || 
                               purchaseOrder.supplier_address_city;
              ...
              return (
                <div className="col-span-2 rounded-lg bg-slate-50 border border-slate-200 p-4">
                  ...
                  <div className="text-sm text-slate-600 space-y-1">
                    {purchaseOrder.supplier_address_street && (
                      <div>{purchaseOrder.supplier_address_street}</div>
                    )}
                    {(purchaseOrder.supplier_address_city || purchaseOrder.supplier_address_state || 
                      purchaseOrder.supplier_address_zip) && (
                      <div>
                        {[
                          purchaseOrder.supplier_address_city,
                          purchaseOrder.supplier_address_state,
                          purchaseOrder.supplier_address_zip
                        ].filter(Boolean).join(', ')}
                      </div>
                    )}
            ...
            <div key={index} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  disabled={!vendorId && !purchaseOrder.supplier_id}
                ...
                >
                  <option value="">
                    {productsLoading ? 'Loading products...' :
                     !vendorId && !purchaseOrder.supplier_id ? 'Select a vendor or supplier first...' :
              ...
              <div className="mt-2 text-right text-sm text-slate-600">
                {line.store_price && line.customer_margin_percent !== undefined && line.customer_margin_percent !== null ? (
                  <>
                    <div className="font-medium text-emerald-700">
                      Customer Amount: ${((line.quantity * line.store_price * (1 - line.customer_margin_percent / 100)).toFixed(2))}
                    </div>
        ...
        <div className="mt-6 border-t border-slate-200 pt-4">
          <div className="flex justify-end">
            <div className="w-80 space-y-2">
              {purchaseOrder.supplier_id && lines.some(line => line.store_price && line.customer_margin_percent !== undefined && line.customer_margin_percent !== null) ? (
                <>
                  ...
                  <div className="flex justify-between text-xs text-slate-500">
                    <span>PO Subtotal (Cost to ICC):</span>
                    <span>${poSubtotal.toFixed(2)}</span>
                  </div>
                  {totalSupplierShare > 0 && (
                    <div className="flex justify-between text-xs text-purple-600 pl-4">
                      <span>└─ Supplier Share:</span>
                      <span>${totalSupplierShare.toFixed(2)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between text-xs text-slate-500 border-t pt-2">
                    <span>ICC Margin:</span>
                    <span>${(subtotal - poSubtotal).toFixed(2)}</span>
                  </div>
                </>
              ) : (
                <>
